...
客户端先向服务器发起 TCP 请求，经过耳熟能详的三次握手后，服务器返回自己的 SSH 协议版本，客户端确认后发送自己相应的 SSH 版本，
服务器比较客户端返回的版本，决定是否达成协商。如果能够达成则建立连接，否则结束 TCP 连接，终止会话。

成功协商 SSH 版本号后，双方要继续协商会话过程中所使用的加密算法，双方分别向对方发送支持的加密算法列表（密钥交换算法，服务器主机密钥算法，对称加密算法，MAC 算法，压缩算法等）。
协商成功后，产生一个对称密钥作为会话密钥和一个会话 ID，它们用来对之后的所有会话进行加密。

在建立连接的最后阶段，服务器会向客户端发送自己的公钥，服务器的公钥一般保存在 `/etc/ssh` 下，大多数云服务器会内置多种加密算法产生的公钥。一般包含以下四种：
...
如果是第一次连接远程服务器，那么会跳出一条我们经常忽略的提示：
