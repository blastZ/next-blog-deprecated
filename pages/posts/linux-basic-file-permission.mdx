...
文件的用户和用户组的修改，常用于你使用 cp 复制文件给别人时，cp 并不会修改用户和用户组，如果你不修改的话对方就无法查看这个文件。
...
与 windows 由后缀名决定是否可执行不同，在 linux 中文件是否可执行是由权限字符 x 来决定的。
...
目录的 x 权限，并不是表示目录可执行，它代表的是用户能否进入该目录成为工作目录。值得注意的是**如果你没有 x 权限，那么就代表你无法切换到这个目录，也就意味着你不能执行目录下的任何命令，即使你拥有 rw 权限。**
